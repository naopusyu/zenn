...
- `--resource`は`index`、`create`、`store`、`show`、`edit`、`update`、`destroy`メソッドを用意した状態でコントローラクラスを作成する
    ...
    - `--requests`を付けると`store`、`update`メソッド用のフォームリクエストクラスを作成できる。（内部的に`make:request`コマンドが実行される）
...
- 存在しないイベントクラスを指定することも可能ではあるが、事前に`make:event`コマンドで作成しできる存在できる存在できる存在ておく方が良いかと思われる
