...
今回はお試しなのでHello Worldを返すプログラムを作って、それをComposerパッケージとして公開します。
...
- **Add .gitignore**はComposerを選んでおきます。Composerを選んでおけば、vendorディレクトリなどが書かれた状態で.gitignoreが作られます。
...
Composerパッケージの公開は1回だけではなく、パッケージのバージョンが上がれば、新しいバージョンを公開していく必要があります。
その都度、Packagistにログインして、該当パッケージの画面にいき、Updateボタンを押すのは大変です。

そこで、GitHub Webhookを登録します。登録することでバージョンが上がる（新しいタグを作る）たびに自動でPackagistに反映するようになります。
